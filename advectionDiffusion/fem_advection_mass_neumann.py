...
V = RestrictedFunctionSpace(W,boundary_set=[1,2])
delta = Constant(1e-4)
...
File('output/fem_advection_normal_eq_neumann_div_{}.pvd'.format(float(nu))).write(divbu)
File('output/fem_advection_normal_eq_neumann_{}.pvd'.format(float(nu))).write(un)
#Dump everything to a pandas dataframe
df = pd.DataFrame({'res': res})
df.to_csv('output/fem_advection_mass_normal_eq_neumann_{}_{}.csv'.format(float(nu),float(n)))
